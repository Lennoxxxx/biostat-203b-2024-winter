...
**About Parquet folder:**
For this task, the parquet folders are placed in the working dictionary `hw3`, please use these commends to create symbolic link.
```{bash}
#| eval: false
ln -s /path/to/labevents_pq labevents_pq
ln -s /path/to/chartevents_pq chartevents_pq
```
...
```{r}
#Read chartevent data
chartevents_dataset <- open_dataset("chartevents_pq", format = "parquet")
  
chartevents_data <- chartevents_dataset %>%
  select(subject_id, stay_id, itemid, charttime, valuenum) %>%
  filter(itemid %in% c(220045, 220181, 220179, 223761, 220210)) 

chartevents_tibble <- chartevents_data %>% collect()

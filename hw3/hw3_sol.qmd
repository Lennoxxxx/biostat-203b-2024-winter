...
selected_procedures$chartdate <- as.POSIXct(selected_procedures$chartdate, tz = "UTC")
...
ggplot() +
  # ADT
  geom_segment(data = adt_transfer_data, aes(x = event_start, xend = event_end, y = 'ADT', yend = 'ADT', 
                                             color = careunit, size = icu_ccu), 
               show.legend = TRUE) +
  scale_size_manual(values = c("ICU/CCU" = 5.0, "Other" = 1.0), guide = FALSE) +
  
  # LAB
  geom_point(data = lab_data, aes(x = charttime, y = 'LAB'), shape = 3, size = 3, show.legend = FALSE) +
  
  # Procedure
  geom_point(data = selected_procedures, aes(x = chartdate, y = 'Procedure', shape = procedure_type), 
  ...
  guides(color = guide_legend(title = "Care Unit", override.aes = list(shape = NA), ncol = 3),
         shape = guide_legend(title = "Procedure", , override.aes = list(linetype = "blank", size = 3), ncol = 1)) +

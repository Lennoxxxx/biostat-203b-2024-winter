...
Attach lab label to the labevents based on `itemid`:
```{r}
labevents_data_label <- labevents_data %>%
  left_join(d_labitems_tble, by = "itemid")
```
For icustays data, select the `subject_id`, `stay_id` and `outtime` column, only keeps data with the latest `outtime` for each `stay_id`:
```{r}
icustays_tble_lab <- icustays_tble %>%
  select(subject_id, hadm_id, stay_id, intime, outtime) %>%
  group_by(stay_id) %>%
  slice_max(order_by = outtime, n = 1) %>%
  ungroup()

```

Join processed labevents data and icustays data to get items for the patients in `icustays_tble`. Restrict to the last available measurement (by `storetime`) before the ICU stay (by `outtime`). For a single `stay_id`, remove duplicated `itemid` by only keeping the item with latest `storetime` and `charttime`:

```{r}
labevents_data_label

  
icu_summarize2 <- icustays_tble_lab %>% 
  group_by(subject_id, hadm_id) %>%
  summarise(same_sh = n() > 1) %>%
  filter(same_sh == TRUE) %>%
  ungroup()


icustays_tble_lab %>% filter(subject_id == 10002428 & hadm_id == 28662225)
```

```{r}
labevents_icu <- labevents_data_label %>%
  inner_join(icustays_tble_lab, by = c("subject_id", "hadm_id")) %>%
...
parquet_data <- chartevents_parquet %>%
  select(subject_id, hadm_id, stay_id, itemid, charttime, storetime, valuenum) %>%
  filter(itemid %in% c(220045, 220181, 220179, 223761, 220210)) 
...
For icustays data, select the `subject_id`, `intime` and `outtime` column, only keeps data with the earliest `intime`:
```{r}
icustays_tble_chart <- icustays_tble %>%
  select(subject_id, hadm_id, stay_id, intime, outtime) %>%
  group_by(stay_id) %>%
...
Join processed chartevents data and icustays data to get items for the patients in `icustays_tble`. Restrict to the the first vital measurement (by `storetime`) between the ICU stay (from `intime` to `outtime`). For a single `stay_id`, remove duplicated `itemid` by only keeping the item with latest `storetime` and `charttime`:
```{r}
chartevents_icu <- chartevents_data %>%
  #semi_join(icustays_tble_chart, by = "subject_id") %>%
  inner_join(icustays_tble_chart, by = c("subject_id", "hadm_id", "stay_id")) %>%
  group_by(stay_id, itemid) %>%
  filter(min(storetime) < outtime) %>%
  filter(min(storetime) > intime) %>%
  slice_max(order_by = storetime, n = 1) %>%
  slice_max(order_by = charttime, n = 1) %>%
  distinct(itemid, .keep_all = TRUE) %>%
  ungroup()
...
# Calculate summary statistics for LOS by each demographic variable
...
# Visualize LOS vs. demographic variables
ggplot(mimic_icu_cohort, aes(x = race, y = los)) + 
  geom_boxplot() + 
  facet_wrap(~insurance + marital_status + gender, scales = 'free_x') +
